...
import { ArrowLeft, ShoppingCart, Info, CheckCircle2, ShieldCheck, Tag, Leaf, Ruler, Flower2, Weight } from "lucide-react";
...
export default function ProductDetailPage() {
  ...
  useEffect(() => {
    const fetchDetail = async () => {
      ...
      } catch { /* fail */ } finally { setIsLoading(false); }
    };
    if (id) fetchDetail();
  }, [id]);
  ...
  const handleAddToCart = async () => {
    ...
    setIsAdding(true);
    try {
      await api.post("/cart", { product_id: product!.id, quantity, note });
